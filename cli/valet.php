...
$app = new Application('Valet Plus Reforged', $version->getVersion());
...
$app->command(
    ...
    function ($withMariadb) {
        ...
        # install directory and start services
...
if (is_dir(VALET_HOME_PATH)) {
    ...
    $app->command(
        ...
        function ($path = null) {
            ...
            info(
                ($path === null
                    ? "This"
                    : "The [{$path}]") . " directory has been added to Valet's paths."
            );
        }
    ...
    $app->command(
        ...
        function ($path = null) {
            ...
            info(
                ($path === null
                    ? "This"
                    : "The [{$path}]") . " directory has been removed from Valet's paths."
            );
        }
     ...
     */
    $app->command(
        'subdomain [action] [name] [--secure] [--proxy]',
        function (
            $action,
            $name,
            $secure,
    ...
    $app->command(
        ...
        function ($domain = null) {
            $url = ($domain
                    ?: Site::host(getcwd())) . '.' . Configuration::read()['domain'];
    ...
    $app->command(
        ...
        function (
            ...
            $service = (isset($supportedServices[$service])
            ...
            if ($run === 'import') {
                ...
                if (Mysql::isDatabaseExists($optional)) {
                    $question = new ConfirmationQuestion(
                        'Database already exists are you sure you want to continue? [y/N] ', FALSE
                    );
                    if (!$helper->ask($input, $output, $question)) {
                        warning('Aborted');
    ...
    $app->command(
        ...
        'Enable proxying for a site instead of handling it with a Valet driver. Useful for SPAs and Swoole applications.'
    );
